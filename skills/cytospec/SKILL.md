...
allowed-tools:
  ...
  - Bash(node {{applets_path}}/assemble.mjs *)
---
...
|---------|-------------------|-------|
| `{{command_prefix}}cytospec new specs/` | "Analyze these spec files" | [new](sub-skills/new.md) |
| `{{command_prefix}}cytospec merge` | "Merge my latest session into the master" | [merge](sub-skills/merge.md) |
| `{{command_prefix}}cytospec view` | "Show me the decision graph" | [view](sub-skills/view.md) |
| `{{command_prefix}}cytospec status` | "What's in my graph?" | [status](sub-skills/status.md) |
| `{{command_prefix}}cytospec` | No argument — assess what the user needs | See routing below |
